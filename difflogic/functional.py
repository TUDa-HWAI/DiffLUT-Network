...
def get_unique_connections_lut6(in_dim, out_dim, device='cuda'):
    assert out_dim * 6 >= in_dim, \
        f'The number of neurons ({out_dim}) must not be smaller than in_dim/6 ({in_dim/6:.1f}).'

    # 使用类似于卷积核滑动窗口（带膨胀/dilation）的思路来生成连接组
    # 这样能确保结构性，并且保证无限提取都不越界。
    conns = [[] for _ in range(6)]
    
    count = 0
    dilation = 1
    
    while count < out_dim:
        for start in range(in_dim):
            if count >= out_dim:
                break
                
            # 模拟6个引脚，通过当前跨度(dilation)选取输入节点的下标
            group = [(start + i * dilation) % in_dim for i in range(6)]
            
            # 我们通过转换为集合来检测是否有冗余节点（即是否有某个 LUT的两个引脚连到了同一个点）
            # 只有当这6个引脚全部分配到不同特征时，才属于有效 Unique 连接
            if len(set(group)) == 6:
                for i in range(6):
                    conns[i].append(group[i])
                count += 1
                
        dilation += 1
        
        # 极端保护：如果特征维度由于自身原因不够拆解，触发保底全随机方案避免死循环
        if dilation >= in_dim:
            while count < out_dim:
                # 保底：如果 in_dim >= 6 则无替换抓取；否则允许重复抓取
                if in_dim >= 6:
                    rp = torch.randperm(in_dim)[:6].tolist()
                else:
                    rp = torch.randint(0, in_dim, (6,)).tolist()
                for i in range(6):
                    conns[i].append(rp[i])
                count += 1

    # 转为 Tensor 结构
    conns = [torch.tensor(c, dtype=torch.int64, device=device) for c in conns]
    
    # 最后统一打乱分配出去
    perm = torch.randperm(out_dim)
    conns = tuple(c[perm] for c in conns)
