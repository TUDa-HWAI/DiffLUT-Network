...
class DiffLUTOptimizer(TorchOptimizer):
    """Optimizer for DiffLUT-Net using a mean-field binary approximation.

    This optimizer does not support multiple parameter groups. All model
    parameters must therefore share the same optimizer settings.
    ...
    def __init__(self, model, train_set_size, lr=1e-9, betas=0.0,
                 prior_lamda=None, num_samples=5, lamda_init=10,
                 lamda_std=0, temperature=1e-10, reweight=1, params=None):
        if lr <= 0.0:
        ...
        # Natural parameter of the binary distribution.
        mixtures_coeff = torch.randint_like(p, 2)
        noise_scale = np.sqrt(lamda_std)
        positive = lamda_init + noise_scale * torch.randn_like(p)
        negative = -lamda_init + noise_scale * torch.randn_like(p)
        self.state['lamda'] = mixtures_coeff * positive + (1 - mixtures_coeff) * negative
        # Expectation parameter of the binary distribution.
        self.state['mu'] = torch.sigmoid(self.state['lamda'])
    ...
    def step(self, closure):
        ...
        if closure is None:
            raise RuntimeError(
                'DiffLUTOptimizer requires the model and loss to be reevaluated inside step()')
        ...
        if defaults['num_samples'] <= 0:
            ...
            w_vector = torch.sigmoid(self.state['lamda'])
            ...
            for _ in range(defaults['num_samples']):  # sampling samples to estimate the gradients
                ...
                rou_vector = torch.log(raw_noise/(1 - raw_noise)) + self.state['lamda']

                w_vector = torch.sigmoid(rou_vector/temperature)
                ...
                scale = (w_vector*(1 - w_vector)+1e-10)/temperature/(self.state['mu']*(1 - self.state['mu'])+1e-10)
                grad_hat.add_(scale * grad)

            grad_hat = grad_hat.mul(defaults['train_set_size'] / defaults['num_samples'])

        kl_grad = reweight * (self.state['lamda'] - self.state['prior_lamda'])

        # Add momentum
        self.state['momentum'] = momentum_beta * self.state['momentum'] + (1-momentum_beta)*(grad_hat + kl_grad)
    ...
    def get_mc_predictions(self, forward_function, inputs, ret_numpy=False, raw_noises=None, *args, **kwargs):
        ...
        if raw_noises is None: # use the mean value (sign) to make predictions
            raw_noises = []
            mean_vector = torch.where(self.state['mu']<=0.5,torch.zeros_like(self.state['mu']),torch.ones_like(self.state['mu']))
        ...
        for raw_noise in raw_noises:
            # Sample a parameter vector:
            vector_to_parameters(raw_noise, parameters)
